...
bool GreaterThanOrEqual(const std::string &condition, const std::string &value, const CSetting *setting)
{
  if (setting == NULL)
    return false;
...
void CSettingConditions::Initialize()
{
  ...
  m_complexConditions.insert(std::pair<std::string, SettingConditionCheck>(">=",                            GreaterThanOrEqual));
}
