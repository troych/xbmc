...
public:
  ...
  bool RegisterMe(void* handle)
  {
    ...
    if (!PERIPHERAL_REGISTER_SYMBOL(m_libKODI_peripheral, PERIPHERAL_feature_category)) return false;
  ...
  JOYSTICK_FEATURE_CATEGORY FeatureCategory(const std::string& strControllerId, const std::string& featureName)
  {
    return PERIPHERAL_feature_category(m_handle, m_callbacks, strControllerId.c_str(), featureName.c_str());
  }
...
protected:
    ...
    JOYSTICK_FEATURE_CATEGORY (*PERIPHERAL_feature_category)(void* handle, CB_PeripheralLib* cb, const char* controllerId, const char* featureName);
