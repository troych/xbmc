...
INPUT_TYPE CDigitalAnalogButtonConverter::GetInputType(const FeatureName& feature) const
{
...
{
  if (GetInputType(feature) == INPUT_TYPE::ANALOG)
...
{
  if (GetInputType(feature) == INPUT_TYPE::DIGITAL)
  {
...
bool CDigitalAnalogButtonConverter::OnAnalogStickMotion(const FeatureName& feature, float x, float y)
{
  return m_handler->OnAnalogStickMotion(feature, x, y);
}

bool CDigitalAnalogButtonConverter::OnAccelerometerMotion(const FeatureName& feature, float x, float y, float z)
{
  return m_handler->OnAccelerometerMotion(feature, x, y, z);
}

bool CDigitalAnalogButtonConverter::IsActivated(const FeatureName& feature) const
{
  return std::find(m_activatedFeatures.begin(), m_activatedFeatures.end(), feature) != m_activatedFeatures.end();
}

void CDigitalAnalogButtonConverter::Activate(const FeatureName& feature)
{
  m_activatedFeatures.push_back(feature);
}

void CDigitalAnalogButtonConverter::Deactivate(const FeatureName& feature)
{
