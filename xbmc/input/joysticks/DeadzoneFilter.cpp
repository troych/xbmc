...
#include "DefaultJoystick.h" // for DEFAULT_CONTROLLER_ID
...
#define RIGHT_STICK_NAME  "rightstick"
...
float CDeadzoneFilter::FilterAxis(unsigned int axisIndex, float axisValue)
{
  ...
  bool bSuccess = GetDeadzone(axisIndex, deadzone, LEFT_STICK_NAME, SETTING_LEFT_STICK_DEADZONE) ||
                  GetDeadzone(axisIndex, deadzone, RIGHT_STICK_NAME, SETTING_RIGHT_STICK_DEADZONE);

  if (bSuccess)
    return ScaleDeadzone(axisValue, deadzone);
...
float CDeadzoneFilter::ScaleDeadzone(float value, float deadzone)
{
